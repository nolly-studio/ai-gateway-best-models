...
export function hasZdr(model: RankedModel): boolean {
  return model.zdr === "all" || model.zdr === "some"
}
...
export function pickCheapRouter(models: RankedModel[]): RankedModel | null {
  ...
  const pool =
    adopted.length > 0
      ? adopted
      : models.filter((model) => hasZdr(model) && isCapable(model))
  return (
    pool
      .toSorted((left, right) => {
...
export function pickFrontier(leaderboard: RankedModel[]): RankedModel | null {
  const withScore = leaderboard.filter(
    (model) => hasZdr(model) && isCapable(model) && model.deepsecScore != null
  )
  ...
  const withOverpay = leaderboard.filter(
    (model) => hasZdr(model) && isCapable(model) && model.overpay != null
  )
  ...
  return (
    leaderboard
      .filter(
        (model) => hasZdr(model) && isCapable(model) && model.spendShare > 0
      )
...
export function pickBangForBuck(models: RankedModel[]): RankedModel | null {
  ...
  const pool =
    qualifying.length > 0
      ? qualifying
      : models.filter((model) => hasZdr(model) && model.deepsecBang != null)
  return (
    pool
      .toSorted((left, right) => {
