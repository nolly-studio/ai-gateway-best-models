    ...
    <article className="overflow-hidden rounded-card bg-surface shadow-card">
      <div className="primitive-card-pad">
        <div className="flex flex-wrap items-center gap-1">
          {roles.map((role) => (
            <span
              className="inline-flex h-5 items-center rounded-chip bg-inset px-1.5 text-xs font-medium text-ink-2"
              key={role.key}
            >
              {role.label}
            </span>
        ...
        <div className="mt-2 flex flex-col gap-1.5">
          <CopyModelId
            ...
            status={status}
          />
          {why.length > 0 || discount != null ? (
            <div className="flex flex-wrap items-center gap-x-2 gap-y-1 text-xs leading-relaxed text-ink-3">
              {why.map((part) => (
                <span className="whitespace-nowrap tabular-nums" key={part}>
                  {part}
                </span>
              ))}
              {discount != null ? (
                <span className="inline-flex h-5 items-center rounded-chip bg-green-tint px-1.5 font-medium text-green tabular-nums">
                  {discount}
                </span>
              ) : null}
            </div>
